...
export default function BrowniesOption() {
    const { browniesMenu } = useContext(LayerContext)

    return (
        <div className='flex'>
            {/* Products */}
            <div className="grid grid-cols-4 w-[73%] px-[15px]">
                {browniesMenu.map((brownie) => (
                    <div key={brownie.id} className='mx-[15px] mb-7.5'>
                        {/* product */}
                        <div>
                            {/* Image */}
                            <img
                            ...
                            <p className="text-[17px] text-[#777777]">{brownie.description}</p>

                            {/* Billing Area */}
                            <div className="flex flex-col gap-5 justify-between mt-6">
                                <span className="text-[#b09c6d] text-[17px] font-bold">
                                    {brownie.currency}{" "}
                                    <span className="font-semibold">{brownie.price}</span>
                                </span>
                                {/* add to cart area  */}
            ...
            {/* cart */}
            <div className="w-[27%] px-[15px] bg-amber-600"></div>
        </div>
    )
}