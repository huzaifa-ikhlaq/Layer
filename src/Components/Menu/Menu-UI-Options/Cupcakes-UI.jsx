...
export default function CupcakesOption() {

  const { cupcakesMenu } = useContext(LayerContext)

  return (
    <div className='flex'>
      {/* Products */}
      <div className="grid grid-cols-4 w-[73%] px-[15px]">
        {cupcakesMenu.map((cupcake) => (
          <div key={cupcake.id} className='mx-[15px] mb-7.5'>
            {/* product */}
            <div>
              {/* Image */}
              <img
              ...
              <p className="text-[17px] text-[#777777]">{cupcake.description}</p>

              {/* Billing Area */}
              <div className="flex flex-col gap-5 justify-between mt-6">
                <span className="text-[#b09c6d] text-[17px] font-bold">
                  {cupcake.currency}{" "}
                  <span className="font-semibold">{cupcake.price}</span>
                </span>
                {/* add to cart area  */}
